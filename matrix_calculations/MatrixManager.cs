...
public class MatrixManager : MonoBehaviour
{
    ...
    [HideInInspector] public int[][] dynamicMatrix; //матрица динамических объектов, 0 - нет преп€тствий, < 0 - статическое (неразрушаемое) преп€тствие, > 0 - id матричного коллайдера динамического объекта
    ...
    public List<Vector3> SearchPath(Vector3 start, Vector3 finish, bool searchShortest, bool castDynamicObjects, int pixelRadius = 0)
    {
        ...
        SearchPath3(coordsA.z, coordsA.x, coordsB.z, coordsB.x, searchShortest, castDynamicObjects); // поиск необработанного пути, путь собираетс€ поклеточно
        if (recPathCoords.Count > 0)
        {
            if (pixelRadius > 0) PathPostProcess_Radius(pixelRadius);// если хотим учитывать радиус объекта (объект больше одной €чейки матрицы)
            PathPostProcess_ReducePoints();// удал€ем средние точки на пр€мых участках пути
            PathPostProcess_StairsRomoving();// удал€ем лесенку - оставл€ем диагональные точки
